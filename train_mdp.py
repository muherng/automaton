...
from generate_mdp import make_program
...
def train_on_data(args,path_to_pretrain = None):
    ...
    if args.seq == 'enc': 
        ...
        src_np = np.load(directory+f'/src_s{args.states}i{args.iterations}.npy',mmap_mode='r')
        tgt_np = np.load(directory+f'/tgt_s{args.states}i{args.iterations}.npy',mmap_mode='r')
...
def seq2seq_training(model, dataset, args, defaults=None, np_data = None): 
    ...
    for epoch in range(args.epochs):
        ...
        for chunk in chunked_dataloader(train_dl, chunk_size=50):
            ...
            file = directory + f'/{args.seq}_{args.kind}s{args.states}i{args.iterations}_acc.npy'
            np.save(file,acc_list)

            file = directory + f'/{args.seq}_{args.kind}s{args.states}i{args.iterations}.pth' 
